...
YourIpCidr=<YOUR_GLOBAL_IP>/32
DomainName=<YOUR_DOMAIN_NAME>
PublicHostedZoneId=<YOUR_PUBLIC_HOSTED_ZONE_ID>
AlbTargetPort=3000
...
```bash
cd ~
git clone https://github.com/<YOUR_GITHUB_ACCOUNT>/aws-webapp-infra-lab.git
cd aws-webapp-infra-lab
```

`<YOUR_GITHUB_ACCOUNT>` は、自分のGitHubアカウント名に置き換えます。
...
# RDS
export AWS_INTRO_SAMPLE_DATABASE_PASSWORD='<YOUR_APP_DB_PASSWORD>'
...
# Application Host
export AWS_INTRO_SAMPLE_HOST='www.<YOUR_DOMAIN_NAME>'

# SES SMTP
export AWS_INTRO_SAMPLE_SMTP_DOMAIN='<YOUR_DOMAIN_NAME>'
...
|  8 | ユーザー登録は成功するが有効化メールが送信されない                             | `AWS_INTRO_SAMPLE_HOST` が未設定で、メール本文のURL生成に失敗している         | `AWS_INTRO_SAMPLE_HOST='www.<YOUR_DOMAIN_NAME>'` を設定してPumaを再起動する       |
