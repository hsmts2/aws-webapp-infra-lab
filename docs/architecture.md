...
Amazon SESはメール送受信に使用します。
ただし、本番アクセス申請やSMTP認証情報の作成など手動対応が必要なため、CloudFormationでは作成せず、手順書とアプリケーション側の環境変数で管理します。
```
...
| 項目 | 書籍 | 本リポジトリ | 理由 |
|---|---|---|---|
| Ruby | 2.6.6 | 2.7.8 | Ruby2.6系は最新bundler/nokogiri等が互換性問題が発生する |
...
| ストレージ | （汎用SSD） | gp3 | 現行の標準的なストレージタイプを使用するため |
...
これらの処理をUserDataに含めると、失敗時の再実行や切り分けが難しくなり、保守性が下がります。
